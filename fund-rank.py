...
def main(argv):
    ...
    if len(sys.argv) == 4:
        ...
        print
        jingzhimin + '\t\t' + jingzhimax + '\t\t' + str(jingzhidif) + '\t' + str(jingzhirise) + '%'
        sys.exit(0)
    ...
    print
    'funds sum:' + str(len(all_funds_list))

    # 4 循环处理每个基金
    for fund in all_funds_list:
        print
        'process fund:\t' + fund[0].encode('gb18030') + '\t' + fund[2].encode('gb18030')
        strfundcode = fund[0]
        # 获取净值
        jingzhimin = get_jingzhi(strfundcode, strsdate)
        jingzhimax = get_jingzhi(strfundcode, stredate)
    ...
    for index in range(len(all_funds_list)):
        ...
        strcontent = str(index + 1) + '\t' + all_funds_list[index][0].encode('gb18030') + '\t' + all_funds_list[index][
            2].encode('gb18030') + \
                     '\t\t' + all_funds_list[index][3].encode('gb18030') + '\t\t' + all_funds_list[index][5].encode(
            'gb18030') + '\t\t' + \
                     all_funds_list[index][6].encode('gb18030') + '\t\t' + str(all_funds_list[index][7]) + '\t' + str(
            all_funds_list[index][8]) + '%\n'
        print
        strcontent
